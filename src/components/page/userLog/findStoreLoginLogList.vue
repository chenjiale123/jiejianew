...
<script>
import { getStoreLog, getLogRespons } from '@/api/log';
...
export default {
  ...
  methods: {
    find(row) {
      this.open = true;
      getLogRespons({ id: row.id }).then(res => {
        console.log(res);
    ...
    getList() {
      this.loading = true;
      const that = this;
      getStoreLog({ pageNum: this.queryParams.pageNum, pageSize: this.queryParams.pageSize })
        .then(res => {
          console.log(res);
    ...
    handleQuery() {
      console.log(this.queryParams.time, '时间');
      getStoreLog({
        loginid: this.queryParams.loginid,
        startTime: this.queryParams.time ? moment(this.queryParams.startTime).format('YYYY-MM-DD HH:mm:ss') : '',
        endTime: this.queryParams.time ? moment(this.queryParams.endTime).format('YYYY-MM-DD HH:mm:ss') : '',
