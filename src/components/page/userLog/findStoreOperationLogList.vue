...
<script>
import { getStoreOpera, getOLogRespons } from '@/api/log';
...
export default {
  ...
  methods: {
    find(row) {
      this.open = true;
      getOLogRespons({ id: row.id }).then(res => {
        console.log(res);
    ...
    getList() {
      this.loading = true;
      const that = this;
      getStoreOpera({ pageNum: this.queryParams.pageNum, pageSize: this.queryParams.pageSize })
        .then(res => {
          console.log(res);
