...
<script>
import { getStoreExcep, getELogRespons } from '@/api/log';
...
export default {
  ...
  methods: {
    find(row) {
      this.open = true;
      getELogRespons({ id: row.id }).then(res => {
        console.log(res);
    ...
    getList() {
      this.loading = true;
      const that = this;
      getStoreExcep({ pageNum: this.queryParams.pageNum, pageSize: this.queryParams.pageSize })
        .then(res => {
          console.log(res);
